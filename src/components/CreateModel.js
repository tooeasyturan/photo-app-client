...
const CreateModel = ({ user, loggedInUser }) => {
  const { handleChange, handleSelect, handleSubmit, values } = useForm(CREATE_MODEL_OPTIONS, submit, validateModel)
  ...
  return (
    <div>
      {/* {createModel()} */}
      <ManageProfilePage user={user} values={values} handleChange={handleChange} handleSubmit={handleSubmit} handleSelect={handleSelect}
      />
    </div>
  )
