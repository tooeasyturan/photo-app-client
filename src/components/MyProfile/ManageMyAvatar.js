...
const ManageMyAvatar = ({ user }) => {
  ...
  const { fetchAvatar, avatar, handleChange, isLoading } = useImageHandling(user, true, file, setIsUpdated)
    ...
    <>
      ...
      {file && isUpdated === false ?
        <Button style={isLoading ? { background: 'none' } : { background: 'red' }}
          fluid size='medium' type='submit' onClick={handleChange}>{isLoading === true ? <Loader incline active /> : 'Confirm avatar change'}</Button> : null}
    </>
  )
}
